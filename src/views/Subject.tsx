...
export function SubjectPage() {
  ...
  const { subjects, topics, deleteSubject, updateSubject } = useStore();
  const subject = subjects.find((s) => s.id === id);
  const subjectTopics = useMemo(
    () => topics.filter((t) => t.subjectId === id),
    [topics, id],
  );
  ...
  const easyCount = subjectTopics.filter(
    (t) => t.currentDifficulty === "easy",
  ).length;
  const medCount = subjectTopics.filter(
    (t) => t.currentDifficulty === "medium",
  ).length;
  const hardCount = subjectTopics.filter(
    (t) => t.currentDifficulty === "hard",
  ).length;
  const relearnCount = subjectTopics.filter(
    (t) => t.state === "relearning",
  ).length;
  const newCount = subjectTopics.filter((t) => t.state === "new").length;
  const mastery =
    subjectTopics.length > 0
      ? Math.round((easyCount / subjectTopics.length) * 100)
      : 0;
        ...
        {editing ? (
          <div className="flex items-start gap-4 w-full">
            <div className="flex flex-col items-center gap-2">
              <SubjectIcon name={editIcon} size={40} />
            ...
            <div className="flex-1 space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1 text-xs rounded-md border border-border text-muted-foreground hover:text-foreground hover:bg-accent"
                >
                  Cancel
                </button>
              </div>
          ...
          <>
            <div className="text-4xl">
              <SubjectIcon name={subject.icon} size={40} />
            </div>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this subject and all{" "}
              {subjectTopics.length} topic
              {subjectTopics.length !== 1 ? "s" : ""} within it. This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      {subjectTopics.length > 0 && (
        <div className="bg-card border border-border rounded-lg p-5 mb-6">
          ...
          <div className="flex gap-1 h-5 rounded overflow-hidden">
            {easyCount > 0 && (
              <div className="bg-sl-easy" style={{ flex: easyCount }} />
            )}
            {medCount > 0 && (
              <div className="bg-sl-medium" style={{ flex: medCount }} />
            )}
            {hardCount > 0 && (
              <div className="bg-sl-hard" style={{ flex: hardCount }} />
            )}
            {relearnCount > 0 && (
              <div className="bg-sl-relearn" style={{ flex: relearnCount }} />
            )}
            {newCount > 0 && (
              <div
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        ...
        {subjectTopics.length === 0 ? (
          ...
          subjectTopics.map((topic) => (
            <div
              key={topic.id}
              className="flex items-center gap-3 px-4 py-2.5 border-b border-border last:border-b-0 text-sm hover:bg-accent/30 transition-colors"
            >
