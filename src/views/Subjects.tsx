...
import { TopicsSubjectsToggle } from "@/components/topics/ToggleSwitch";
import { SubjectIcon } from "@/components/subjects/SubjectIcon";

export function SubjectsPage() {
  const router = useRouter();
  const { user } = useAuth();
  const { subjects, topics, refreshAll } = useStore();
  const [showCreate, setShowCreate] = useState(false);

  // Refresh data from the server on mount so agent-created items appear
  useEffect(() => {
    if (user) refreshAll(user.id);
  }, [user, refreshAll]);

  const handleToggleChange = useCallback(
    (mode: "topics" | "subjects") => {
      if (mode === "topics") {
        router.push("/topics");
      }
    },
  ...
  return (
    <PageShell>
      <div className="flex items-center mb-6">
        <TopicsSubjectsToggle value="subjects" onChange={handleToggleChange} />
      </div>
      ...
      {showCreate && (
        <CreateSubjectModal onClose={() => setShowCreate(false)} />
      )}
    </PageShell>
  );
}
